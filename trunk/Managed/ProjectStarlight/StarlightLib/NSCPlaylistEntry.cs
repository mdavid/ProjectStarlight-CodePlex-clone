#region License Header
/**
* 
* This license governs use of the accompanying software. If you use the software, you
* accept this license. If you do not accept the license, do not use the software.
*
* 1. Definitions
* The terms "reproduce," "reproduction," "derivative works," and "distribution" have the
* same meaning here as under U.S. copyright law.
* A "contribution" is the original software, or any additions or changes to the software.
* A "contributor" is any person that distributes its contribution under this license.
* "Licensed patents" are a contributor's patent claims that read directly on its contribution.
* 
* 2. Grant of Rights
* (A) Copyright Grant- Subject to the terms of this license, including the license conditions and limitations in section 3, each contributor grants you a non-exclusive, worldwide, royalty-free copyright license to reproduce its contribution, prepare derivative works of its contribution, and distribute its contribution or any derivative works that you create.
* (B) Patent Grant- Subject to the terms of this license, including the license conditions and limitations in section 3, each contributor grants you a non-exclusive, worldwide, royalty-free license under its licensed patents to make, have made, use, sell, offer for sale, import, and/or otherwise dispose of its contribution in the software or derivative works of the contribution in the software.
* 
* 3. Conditions and Limitations
* (A) No Trademark License- This license does not grant you rights to use any contributors' name, logo, or trademarks.
* (B) If you bring a patent claim against any contributor over patents that you claim are infringed by the software, your patent license from such contributor to the software ends automatically.
* (C) If you distribute any portion of the software, you must retain all copyright, patent, trademark, and attribution notices that are present in the software.
* (D) If you distribute any portion of the software in source code form, you may do so only under this license by including a complete copy of this license with your distribution. If you distribute any portion of the software in compiled or object code form, you may only do so under a license that complies with this license.
* (E) The software is licensed "as-is." You bear the risk of using it. The contributors give no express warranties, guarantees or conditions. You may have additional consumer rights under your local laws which this license cannot change. To the extent permitted under your local laws, the contributors exclude the implied warranties of merchantability, fitness for a particular purpose and non-infringement.
*
*/
#endregion

using System;
using System.Collections.Generic;
using System.Net;
using System.Windows;
using System.Windows.Controls;
using System.Windows.Documents;
using System.Windows.Ink;
using System.Windows.Input;
using System.Windows.Media;
using System.Windows.Media.Animation;
using System.Windows.Shapes;
using System.Windows.Threading;
using Starlight.ASF;
using Starlight.PacketSource;

namespace Starlight.Lib
{
    /// <summary>
    /// A playlist entry that instructs the player to load a MediaStreamSource wrapping a 
    /// multicast stream.  Because a multicast broadcast may switch between multiple streams,
    /// this class creates a MediaStreamSource per NSC stream, and listens to the packet source
    /// to determine when to switch between these stream sources.
    /// </summary>
    public class NSCPlaylistEntry : PlaylistEntry
    {
        private string nscUri;
        private MSBPushPacketSource packetSource;
        private Dictionary<Header, ASFMediaStreamSource> streams = new Dictionary<Header, ASFMediaStreamSource>();
        private ASFMediaStreamSource currentStream;
        private bool needToChangeStream = false;
        private Header lastStreamHeader;
        private MediaElement player;
        private Dispatcher dispatcher;

        public NSCPlaylistEntry(string nscUri)
        {
            this.nscUri = nscUri;
        }

        public override void InitAsync()
        {
            OnInitCompleted();
        }

        public override void SwitchTo(Dictionary<string, object> bridgeContext, MediaElement player)
        {
            MulticastController bridge = (MulticastController)bridgeContext[MulticastController.KEY_BRIDGE];
            PushSourceController pushControl = (PushSourceController)bridgeContext[MulticastController.KEY_PUSH_SOURCE_CONTROLLER];

            //TODO:  Make the NSC fetching happen asynchronously.
            NSC.NSC nsc = new NSC.NSCParser().ParseNSC(pushControl.FetchNSC(nscUri));

            packetSource = new MSBPushPacketSource(nsc);
            packetSource.PacketReceived += this.OnPacket;
            lock (streams)
            {
                //Create a media stream source per stream described in the NSC.
                foreach (PacketSource.Header header in packetSource.AllHeaders)
                {
                    StreamSplitPacketSource subPacketSource = new StreamSplitPacketSource(packetSource, header);
                    ASFMediaStreamSource mss = new ASFMediaStreamSource(header, subPacketSource);
                    streams[header] = mss;
                }
            }
            lastStreamHeader = null;
            currentStream = null;
            this.player = player;
            this.dispatcher = player.Dispatcher;
            packetSource.StartTimer();
            pushControl.StartPush(nsc.Address, nsc.Port, packetSource);
        }

        public override void Leaving(Dictionary<string, object> bridgeContext, MediaElement mediaElement)
        {
            PushSourceController pushControl = (PushSourceController)bridgeContext[MulticastController.KEY_PUSH_SOURCE_CONTROLLER];
            packetSource.StopTimer();
            packetSource.Close();
            pushControl.StopPush();
        }

        private void OnPacket(Packet p)
        {
            if (lastStreamHeader != p.PacketHeader)
            {
                needToChangeStream = true;
                lastStreamHeader = p.PacketHeader;
            }

            if (needToChangeStream)
            {

                lock (streams)
                {
                    ASF.ASFMediaStreamSource nextStream = streams[lastStreamHeader];
                    if (currentStream == null || !currentStream.HasData)
                    {
                        needToChangeStream = false;
                        if (currentStream != nextStream)
                        {
                            ASFMediaStreamSource oldCurrent = currentStream;

                            currentStream = nextStream;
                            DispatcherOperation op = dispatcher.BeginInvoke(delegate()
                            {
                                player.SetSource(currentStream);
                                if (oldCurrent != null)
                                {
                                    lock (streams)
                                    {
                                        streams[oldCurrent.Header] = oldCurrent.CreateReplacementStream();
                                    }
                                }
                            });
                        }

                    }
                }
            }
        }

        public ASFMediaStreamSource MediaStreamSource
        {
            get { return currentStream; }
        }
    }
}
